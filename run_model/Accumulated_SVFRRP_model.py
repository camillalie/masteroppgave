...
def SVFRRP_model(sets, params):
    ...
    emissions_t1 = model.addVars(T1, vtype=gp.GRB.CONTINUOUS, lb=0, name="emissions_t1")
    emissions_t2_w = model.addVars(T2, O, vtype=gp.GRB.CONTINUOUS, lb=0, name="emissions_t2")
    model.update()
    ...
    # Constraint 5.8
    # Calculate total emissions for each period t in T1
    # for t in T1:
    ...
    #                     total_emissions += emissions_t1[t]  # Accumulate emissions for period t
    #     total_emissions_per_t[t] = total_emissions  # Store total emissions for period t

    # # Calculate total emissions for each period t in T2 and add constraints
    # for w in O:             
    #     for t in T2:
    #         total_emissions = gp.LinExpr()
    #         for s in S:
    ...
    #                     for r in R_s:
    #                         if f == 1 or f == 2:
    #                             emissions_t2_w[t,w] += emissions[s, r] * weekly_routes2_s_a_f_r_t_w[s, a, f, r, t, w] * week_to_t
    #                         elif f == 5:
    #                             emissions_t2_w[t,w] += emissions[s, r] * mgoEm_to_b30 * weekly_routes2_s_a_f_r_t_w[s, a, f, r, t, w] * week_to_t / emission_factor**t
    #                         else: 
    #                             emissions_t2_w[t,w] += emissions[s, r] * weekly_routes2_s_a_f_r_t_w[s, a, f, r, t, w] * week_to_t / emission_factor**t
    #                         total_emissions += emissions_t2_w[t,w]  # Accumulate emissions for period t
    #         total_emissions_per_t[t] = total_emissions  # Store total emissions for period t

    #     # Add constraint for total emissions for each period t in T2
    #     tot_emissions = gp.LinExpr()

    #     for t in T:
    #         tot_emissions += total_emissions_per_t[t]
    #     model.addConstr(
    ...
    for t in T1:
        ...
        total_emissions_per_t[t] = emissions_t1[t]
    ...
    for w in O:             
        for t in T2:
            ...
            total_emissions_per_t[t] = emissions_t2_w[t,w]
    ...
    # Constraint 5.23
    # for w in O:  
    #     emissions_S2 = gp.LinExpr()         
    #     for t in T2:
    #         total_emissions_t = gp.LinExpr()
    #         if (t) in max_em_period:
    #             for s in S:
    #                 for a in A:
    #                     for f in F_s:
    ...
    #     model.addConstr(
    #         total_emissions + emissions_S1 <= max_emissions_total,
    #         name= f'emissions_period_constraint_s2_{w}'
    # )
    # total_overall_emissions = total_emissions + emissions_S1    
...
if model.status == gp.GRB.OPTIMAL:
    ...
    print('Overall emissions: ', total_overall_emissions)
    for t in T:
