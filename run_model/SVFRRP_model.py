...
def SVFRRP_model(sets, params):
    ...
    indices_retro2 = [(1, 9, a, t) for a in A 
                      for idx, t in enumerate(sorted(T2)[:-1]) if idx < len(T2) - 1] + [(2, 10, a, t) for a in A 
                                                                           for idx, t in enumerate(sorted(T2)[:-1]) if idx < len(T2) - 2]
    ...
    indices_newpsv_2 = [(s, t, w) for s in S if (s not in not_buy_new) for t in sorted(T2)[:-1] for w in O]
    ...
    # stage 2 variables
    psv2_s_a_t_w = model.addVars(S, A, sorted(T2)[:-1], O, vtype=gp.GRB.INTEGER, lb=0, name="xsat2")                                   # x
    ...
    weekly_routes2_s_a_f_r_t_w = model.addVars(S, A,F_s, R_s, sorted(T2)[:-1], O, vtype=gp.GRB.INTEGER, name="weekly_routes2_s_a_f_r_t_w")            #z^T
    ...
    total_cost_per_t_s2 = {
        (t, w): (probability[w] * (gp.quicksum(
                    ...
                    (fuel_cost2[f, r, t, w]) / eac**(t-1) * weekly_routes2_s_a_f_r_t_w[s,a,f,r,t, w] * week_to_t
                    for s in S for a in A for f in F_s for r in R_s
                ))
                        ...
                        for f in F_s:
                            if (f,s) in compatibility_fs:
                                model.addConstr(
                                    ...
                                    name=f'compatibility_const_s2_{f}_{s}_{w}_{f}'
                                )
                            else: 
                                model.addConstr(
    ...
    for w in O:
        for s in S:
            for a in A:
                t = sorted(T2)[-1]
                if (s,a,t,w) in scrap_psv2_s_a_t_w:
                    psvs_in_last_t = psv2_s_a_t_w[s,a,t,w]
                    model.addConstr(
                            psvs_in_last_t <= scrap_psv2_s_a_t_w[s,a,t,w], 
