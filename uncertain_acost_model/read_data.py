...
sheet_dictionaries = {}
file_path = 'Parameterdata-2.xlsx'


def get_parameters(filename):

    sheet_names = {'Cost of retrofitting 1', 'Cost of retrofitting 2', 'Cost of newbuilding 1', 'Cost of newbuilding 2', 'Cost of fuel 1', 'Cost of fuel 2','Time used',
    ...
    for sheet in sheet_names:
        ...
        for index, row in df.iterrows():
            ...
            # Check if 'Value' column exists before dropping it
            if isinstance(row, pd.DataFrame) and 'Value' in row.columns:
                key_tuple = tuple(row.drop('Value'))
            elif isinstance(row, pd.Series) and 'Value' in row.index:
                key_tuple = tuple(row.drop('Value'))
            else:
                key_tuple = tuple(row)
            #key_tuple = tuple(row.drop('Value'))
...
# print(parameters['Cost of newbuilding 2'])
